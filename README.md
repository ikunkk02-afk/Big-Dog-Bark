...
- 蓄能时间线(20 Tick = 1 秒,统一 Tick 常量):
  ...
  - `16 ～ 39 Tick`(0.8 ～ 1.95 秒):可以发射,威力较低(伤害约 6);
  - `40 ～ 69 Tick`(2.0 ～ 3.45 秒):**蓄能完成**,播放一次 ready 音效 + 动作栏“蓄能完成”,正常高威力(伤害约 10);
  - `70 ～ 89 Tick`(3.5 ～ 4.45 秒):**危险过载区间**,70 Tick 时警告一次(“警告:蓄能即将过载!”+ 警告粒子),仍可松开发射(伤害约 13.4);
  - `90 Tick`(4.5 秒):**自动炸膛 + 强行发射**,不需要松手——玩家附近产生**原版爆炸**(威力 3,不破坏地形、不点火),播放原版爆炸声与爆炸粒子,玩家受到约 7 点伤害和明显击退,**同时过载能量以最大威力强行射出**(伤害 16 的声波),进入 **3 秒冷却**,动作栏提示“蓄能过载!大狗炸膛强行发射了!”。
- **HUD 蓄能进度条**:蓄能时在准心下方显示细进度条(比准心略小),绿色 = 蓄能完成,黄色 = 危险过载区间,红色 = 即将炸膛;炸膛/发射冷却期间自动隐藏。蓄能音效跟随玩家移动(不会停留在原地),松开/发射/炸膛时自动停止。
- **发射方式:声波冲击(大狗本体不离开玩家)**:
  - 松开右键后,从玩家眼前(狗嘴方向)射出**音波冲击** `big_dog_bark:launched_big_dog`(参考原版 1.21.1 坚守者声波:sonic_boom 扩散环粒子在路径上留下一串波环,形似激光/声波通道;实体本身隐形);
  - 波前以 1 格/tick 匀速推进约 45 格(2.25 秒),沿途**以三维圆锥形方式摧毁方块**:以发射点为顶点、飞行方向为轴、绕轴 18° 半角扩散,水平/垂直/斜向任一方位均匀扩散;整条声波路径经过的圆锥区域全部破坏(爆炸抗性低于 1000 的方块,不掉落;流体、基岩、黑曜石保留)——朝天射就是一个竖直锥,水平射就是一个喇叭口;
  - 沿途对圆锥范围内的**生物**造成冲撞伤害(无视护甲的音波伤害,蓄能越高伤害越高:6 → 16)与击退,每个生物只结算一次,不伤害发射者本人;伤害判定与方块破坏同一三维锥体;
  - **大狗不消耗、不离开玩家手中**,可反复蓄能发射;发射后 1 秒短冷却限制射速;
  - 音波伤害源为原版 `sonic_boom`(坚守者同款),不产生爆炸、不点燃目标。
